...
public class ThreadByUserByLastMessage {
  ...
  /**
   * The method decides which thread name and image to assign to the {@link ThreadByUserByLastMessage}
   * object. Because the data is denormalized we have to compute that ahead of time. Resolving happens
   * in the following manner:
   * <ul>
   *   <li>If it is a group thread then return the name of the thread and its picture
   *   because it is uniform across all participants chats</li>
   *   <li>If the user we are preparing it for is not the author then set the message
   *   author's name and picture</li>
   *   <li>If none of those conditions are true, then it means we are inserting
   *   the message for the current user and since it is not a group thread,
   *   we know that there is exactly two participants, namely the author and on the inverse side
   *   the user with who we initiated the thread. Hence, we find the user UDT object that
   *   is not the author and insert return its details. In case the inverse side
   *   has not been found, then we have to remove the corrupted thread that contains only author
   *   and throw {@link CorruptedThreadException} so that the client can delete the thread
   *   from its local cache</li>
   * </ul>
   *
   ...
   * @param toUser - {@link UserUDT} model, participant for who we are trying to construct the object.
   * @return {@link String[]} of size 2, where result[0] contains thread name and
   * result[1] contains thread picture thumbnail.
   */
